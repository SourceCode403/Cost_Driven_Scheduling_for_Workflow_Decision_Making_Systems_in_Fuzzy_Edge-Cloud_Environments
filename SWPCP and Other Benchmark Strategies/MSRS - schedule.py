...
from Data_process import Read_workflows, Pre_Process, Toposort
...
class Chromosome(object):
    def __init__(self, DC, TS, SA):
        ...
        self.tran_qcost = (0.000, 0.000, 0.000)
...
def pop_init():
    ...
    for i in range(pop_size):
        ...
        for _ in range(workflow_taskN):
            TS.append(round(random.uniform(0, workflow_taskN / 2), 1))
        ...
        elif cmp_fitness(population[i], elite):
            elite = copy.deepcopy(population[i])
...
def pop_evolve():
    ...
    # global now
    for i in range(pop_size):
        for j in range(dataCenterN):
            population[i].DC[j] = random.randint(0, serverN - 1)
        for j in range(workflow_taskN):
            population[i].TS[j] = round(random.uniform(0, workflow_taskN / 2), 1)
            population[i].SA[j] = random.randint(0, dataCenterN - 1)
        population[i] = Chromosome(population[i].DC, population[i].TS, population[i].SA)
        schedule(population[i])
        if cmp_fitness(population[i], elite):
            elite = copy.deepcopy(population[i])
            # now = generation


def MSRS(count):
    # Mult-workflow Scheduling strategy based on Random Searching
    global population
    global elite
    global generation
    # global now
    ...
    generation = 1
    pop_init()
    Print_Result(elite, iteration_cost, iteration_time, result, True, 0)
    while generation <= max_iter:
        # if generation - now > 100:
        #     break
        pop_evolve()
        Print_Result(elite, iteration_cost, iteration_time, result, True, generation)
        # for i in range(serverN):
        #     print(g_best.server[i].start_time, g_best.server[i].end_time, g_best.server[i].rent_time)
        print("-----------------------------------------")
        generation = generation + 1
    Print_Result(elite, iteration_cost, iteration_time, result, False, count)
    print("----------------------------------------------------------------------------------------------------------")
...
if __name__ == '__main__':
    ...
    for workflow_i in range(1, len(taskN)):
    ...
    for k in range(N):
        ...
        elite = particle
        MSRS(k + 1)
        sum_cost = fuzzy_sum(sum_cost, elite.run_cost)
        average = fuzzy_divide(sum_cost, N)
        if fuzzy_fitness(worst) < fuzzy_fitness(elite.run_cost):
            worst = elite.run_cost
            worst_makespan = elite.makespan
            worst_deadline = elite.meet_deadline
        if optimal == (-1, -1, -1) or fuzzy_fitness(optimal) > fuzzy_fitness(elite.run_cost):
            optimal = elite.run_cost
            optimal_makespan = elite.makespan
            optimal_deadline = elite.meet_deadline
    ...
    Output_File(Output_path, "MSRS", base, iteration_cost, iteration_time, result)
